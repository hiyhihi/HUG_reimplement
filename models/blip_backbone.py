...
class BLIPBackbone(nn.Module):
    ...
    def extract_image_features_lavis(
        ...
        with torch.cuda.amp.autocast(dtype=torch.float16):
            image_embeds = self.blip_model.ln_vision(
                self.blip_model.visual_encoder(pixel_values)
            )
            ...
            if query_embeds is None:
                query_embeds = self.blip_model.query_tokens.expand(
    ...
    def extract_image_features(
        ...
        """
        image_embeds = self.extract_image_features_lavis(pixel_values, query_embeds = query_embeds)
        return image_embeds
...
# truncate/pad về 32 tokens
        ...
        if text_embeds.size(1) >= 32:
            ...
            pad_size = 32 - text_embeds.size(1)
            ...
            text_embeds = torch.cat([text_embeds, pad], dim=1)
    ...
    def extract_multimodal_features(
        ...
        # Extract features using LAVIS multimodal mode
        return self.extract_multimodal_features_lavis(
            pixel_values,
            input_ids,
            attention_mask,
            query_embeds = query_embeds
