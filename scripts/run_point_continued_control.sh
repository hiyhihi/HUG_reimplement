...
# Clean-only continuation control for point_robust. It warm-starts from the
# same Point checkpoint and matches batch/LR/epochs/early stopping, but trains
# only the original clean symmetric InfoNCE objective.
ROOT_DIR=$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)
DATA_ROOT=${DATA_ROOT:-"$ROOT_DIR/data/fashion-iq"}
VENV=${VENV:-"$ROOT_DIR/ref/LAVIS/.venv/bin/activate"}
RESULT_ROOT=${RESULT_ROOT:-"$ROOT_DIR/results/point_continued_control_v1"}
CHECKPOINT_ROOT=${CHECKPOINT_ROOT:-"$ROOT_DIR/checkpoints/point_continued_control_v1"}
...
RUN_DIR="$CHECKPOINT_ROOT/point_continued/$CATEGORY/seed$SEED"
LOG_FILE="$RESULT_ROOT/logs/point_continued_${CATEGORY}_seed${SEED}.log"
CLEAN_FILE="$RESULT_ROOT/clean/point_continued_${CATEGORY}_seed${SEED}.json"
ROBUST_DIR="$RESULT_ROOT/robustness"
ROBUST_MARKER="$ROBUST_DIR/.point_continued_${CATEGORY}_seed${SEED}.complete"
...
if [[ -f "$RUN_DIR/checkpoint_final.pth" ]]; then
  ...
  python -u train.py \
    ...
    --recipe point_continued --batch_size "$BATCH_SIZE" --num_epochs "$EPOCHS" \
